...
import { ThumbsDown, ThumbsUp, Sparkles, Film, Tv, RefreshCw, Eye, Clock, Check, X, ChevronDown } from 'lucide-react';
...
export function DiscoveryScreen() {
  ...
  const loadStreamingProviders = async () => {
    ...
    try {
      const data = await apiCall(`/tmdb/watch-providers/${item.media_type}/${item.id}`);
  ...
  const handleSwipe = async (ratingType: 'thumbs_down' | 'thumbs_up' | 'two_thumbs_up') => {
      ...
      try {
        await Promise.all([
          apiCall('/queue', {
            ...
            // Silently handle "already in queue" - not an error
            console.log('Queue add result:', err.message);
          }),
          // Save rating with Netflix-style rating type and streaming service
          apiCall('/rate', {
            method: 'POST',
            body: JSON.stringify({
              itemId: item.id,
    ...
    setTimeout(() => {
      ...
      // Reset watched status and streaming service to default after each swipe
